package club.hero.controller;
import java.util.List;

import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;
import com.alibaba.dubbo.config.annotation.Reference;
import club.hero.pojo.Blogtype;
import club.hero.service.BlogtypeService;

import entity.PageResult;
import entity.Result;
/**
 * controller
 * @author Administrator
 *
 */
@RestController
@RequestMapping("/blogtype")
public class BlogtypeController {

	@Reference
	private BlogtypeService blogtypeService;
	
	/**
	 * 返回全部列表
	 * @return
	 */
	@RequestMapping("/findAll")
	public List<Blogtype> findAll(){			
		return blogtypeService.findAll();
	}
	
	
	/**
	 * 返回全部列表
	 * @return
	 */
	@RequestMapping("/findPage")
	public PageResult  findPage(int page,int rows){			
		return blogtypeService.findPage(page, rows);
	}
	
	/**
	 * 增加
	 * @param blogtype
	 * @return
	 */
	@RequestMapping("/add")
	public Result add(@RequestBody Blogtype blogtype){
		try {
			blogtypeService.add(blogtype);
			return new Result(true, "增加成功");
		} catch (Exception e) {
			e.printStackTrace();
			return new Result(false, "增加失败");
		}
	}
	
	/**
	 * 修改
	 * @param blogtype
	 * @return
	 */
	@RequestMapping("/update")
	public Result update(@RequestBody Blogtype blogtype){
		try {
			blogtypeService.update(blogtype);
			return new Result(true, "修改成功");
		} catch (Exception e) {
			e.printStackTrace();
			return new Result(false, "修改失败");
		}
	}	
	
	/**
	 * 获取实体
	 * @param id
	 * @return
	 */
	@RequestMapping("/findOne")
	public Blogtype findOne(Integer id){
		return blogtypeService.findOne(id);		
	}
	
	/**
	 * 批量删除
	 * @param ids
	 * @return
	 */
	@RequestMapping("/delete")
	public Result delete(Integer [] ids){
		try {
			blogtypeService.delete(ids);
			return new Result(true, "删除成功"); 
		} catch (Exception e) {
			e.printStackTrace();
			return new Result(false, "删除失败");
		}
	}
	
		/**
	 * 查询+分页
	 * @param brand
	 * @param page
	 * @param rows
	 * @return
	 */
	@RequestMapping("/search")
	public PageResult search(@RequestBody Blogtype blogtype, int page, int rows  ){
		return blogtypeService.findPage(blogtype, page, rows);		
	}
	
}
